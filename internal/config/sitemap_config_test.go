...
func TestDeltaSitemapSelectionConfigJSONContract(t *testing.T) {
	plan := DeltaPlanConfig{
		...
		SitemapSelection: &DeltaSitemapSelection{
			...
			EventSelected:                      30,
			EventDeferred:                      1,
			PublishedDifferenceTotal:           31,
			ActionableTotal:                    1,
			StableAcknowledgedTotal:            30,
			SelectedTotal:                      51,
			CanarySelected:                     50,
			SelectionComplete:                  false,
			PublicationHeld:                    true,
			StabilityOlderSessionID:            "raw-delta-older",
			StabilityNewerSessionID:            "raw-delta",
			StabilityProofDigest:               "0123456789abcdef",
			StabilityLegacyCompletePair:        true,
